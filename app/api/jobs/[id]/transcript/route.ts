...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // 2. Look up the job and verify ownership in one query. The secret key
  // bypasses RLS, so the user_id filter is what keeps one user from
  // downloading another user's transcript by guessing a uuid.
